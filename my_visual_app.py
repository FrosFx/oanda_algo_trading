...
SPREADS = os.path.join(CURRENT_DIR, csv_name)

# Temporary
csv_name = "GBPUSD_spreads.csv"


pn.extension(sizing_mode="stretch_width")
prices_df = pd.read_csv(SPREADS, parse_dates=["date"], index_col="date")
...
freq = pn.widgets.Select(
    name="frequency",
    options=["1min", "5min", "10min", "15min", "20min", "30min", "45min", "1H"],
)
...
def plot_cover_chart(symbol, freq):
    ...
    return df_to_plot.hvplot.bar(
        x="hour",
        y=["covered", "not_covered"],
        stacked=True,
        color=["green", "red"],
        legend="top",
        ylabel="% of covered spread",
    )
...
pn.panel(pn.bind(plot_cover_chart, symbol, freq), height=400).servable(
    title="Comparison of cover spreads"
)
